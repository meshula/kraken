...
class KLOperator(Operator):
    ...
    def __init__(self, name, solverTypeName, extension, metaData=None, addSolverTypeToName=True):
        if addSolverTypeToName:
            name += "_"+solverTypeName
    ...
    def getDefaultValue(self, name, RTValDataType, mode="arg"):
        ...
        if self.solverRTVal.defaultValues.has("Boolean", name).getSimpleType():
            ...
            if name in self.inputs: #mode == "arg":  #Only report a warning if default value is not provided for arg
