...
class Builder(Builder):
    ...
    def generateKLCode(self):
        ...
        kl += ["  Index i = 0;"]
        kl += ["  for (name, index in this.directDriveObject3Ds)  //Should iterate in insert order"]
        kl += ["    this.setObject3DGlobalMat44ById(index, this.directDriveObject3DValues[i++]);"]
        kl += ["  }"]
        kl += [""]
        kl += ["  {"]
        if self.__profilingFrames > 0:
...
#        kl += ["      report(\"Debug: %s.setObject3DGlobalMat44ById: name: \"+this._KrakenItem[uniqueId].name+\" uniqueId: \"+uniqueId+\" value: \"+value);" % self.getKLExtensionName()]
        ...
        kl += ["            continue;"]
        kl += ["        }"]
        kl += ["        control_joint = KrakenJoint(this._KrakenJoint[control.glue_joint_index]);"]
        kl += ["        control.global = control_joint.global * control.glue_offset;"]
        ...
        kl += ["            {"]
        kl += ["                space_joint = KrakenJoint(this._KrakenJoint[space.glue_joint_index]);"]
        kl += ["                space_global = space_joint.global * space.glue_offset;"]
        kl += ["            }"]
        kl += ["            else"]
        kl += ["            {"]
        kl += ["                space_global = control_joint.global * control.glue_offset;"]
        kl += ["            }"]
        kl += ["            control.local = space_global.inverse() * control.global;"]
        kl += ["        }"]
        kl += ["        else"]
        kl += ["        {"]
        kl += ["            control.local = Mat44(control.global);"]
        kl += ["        }"]
        kl += ["        if (this.debugIter)"]
        kl += ["        {"]
        ...
        kl += [""]
        kl += ["inline function String[] %s.setGlueControlIDsByNames!(in String names[], in Boolean append) {" % self.getKLExtensionName()]
        kl += ["  // Not meant to be fast!"]
        kl += ["  if (!append)"]
        ...
        kl += ["    if (id == -1)"]
        kl += ["        report(\"Warning: %s.setGlueControlIDsByNames joint \\\"\"+names[n]+\"\\\" not found in rig.\");" % self.getKLExtensionName()]
        kl += ["    else"]
        kl += ["        namesInOrder.push(names[n]);"]
        kl += ["        this.glueControlIDs.push(id);"]
        kl += ["        SInt32 glue_joint_index = KrakenObject3D(this._KrakenItem[id]).glue_joint_index;"]
